...
-- Fecha: 5/23/2023
-- Descripción: Este Stored Procedure se adaptó para mostrar el problema del unrepeatable read.
-----------------------------------------------------------
...
BEGIN
	...
	/*
		Puede ocurrir el problema del Unrepeatable Read en la tabla de saldosDistribucion. Al hacer el procesamiento grueso de
		la transacción antes del begin transaction, acortamos el tiempo y el procesamiento dentro de la transacción. Sin embargo,
		el cálculo del descuento por el saldo se hace con base en el monto que haya en el momento donde se hace todo el 
		procesamiento. Si dos transacciones realizan este procesamiento al mismo tiempo, o una inmediatamente después de la otra, 
		van a leer el mismo saldo disponible para ambas. Si este es el caso, cada una puede pensar que puede usar la totalidad 
		del saldo para pagar su recolección. Luego al momento de actualizar el saldo, se resta el monto que leen dos veces, 
		por lo que queda una cantidad negativa en el saldo.	Esto es el problema del unrepeatable read porque la segunda 
		transacción primero lee el valor original, pero cuando hace el update, lee el valor actual del saldo,
		el cual es diferente al valor original que leyó al inicio.
	*/

	-- T1: empieza primero
	-- se lee el valor de saldo 600. El costo de T1 es 1400. Puede gastar todo el saldo, entonces lo agarra todo, 600
