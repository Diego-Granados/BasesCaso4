...
VALUES (SCOPE_IDENTITY(),
	FLOOR(1 + RAND()*1000),
	(SELECT TOP 1 recolectorId FROM recolectores ORDER BY NEWID()), FLOOR(1 + RAND()* 10) * 50, FLOOR(1 + RAND()*1000), 7, 4, 1, 1, GETDATE(), NULL, 'computer01', 'user01', 0x0);
SET @contador = @contador + 1
END
...
select *, nombres.nombreBase from desechos inner join nombres ON desechos.nombreId = nombres.nombreId

select *, nombres.nombreBase from tiposdesechos inner join nombres ON tiposdesechos.nombreId = nombres.nombreId